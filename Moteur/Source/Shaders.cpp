...
void Shaders::CreateSignature() {
 ...
 /*D3D12_FEATURE_DATA_ROOT_SIGNATURE featureData = {};
    ...
    if (FAILED(Engine::Device->CheckFeatureSupport(D3D12_FEATURE_ROOT_SIGNATURE, &featureData, sizeof(featureData))))
    {
    ...
    /*
    * Cr�ation d'un descriptor table
    * cbvTable.Init(a, b, c) :
    ...
    */
    ...
    // Liste des diff�rent Descriptor Range
    CD3DX12_DESCRIPTOR_RANGE descriptorRange[]{
        cbvTable
    };

    /*
    * Tableau des param�tres de la signature racine (ici 1 seul)
    * il existe 3 types de param�tres diff�rents : root constant, root descriptor et descriptor table
    */
    CD3DX12_ROOT_PARAMETER slotRootParameter = CD3DX12_ROOT_PARAMETER();

    // Initialisation des param�tres de la signature racine
    ...
    // Transformation de la description en une structure de donn�es qui peut �tre utilis�e pour cr�er la signature racine
    ID3DBlob* serializedRootSig = nullptr;
    D3D12SerializeRootSignature(&rootSigDesc, D3D_ROOT_SIGNATURE_VERSION_1, &serializedRootSig, nullptr);

    // Cr�ation de la signature racine
...
std::vector<UINT8> Shaders::LoadFromFile(const std::wstring& filePath)
{
    ...
    CoInitialize(nullptr); // Initialise COM si ce n'est pas d�j� fait.

    // Cr�e une instance de l'usine WIC
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                if (SUCCEEDED(hr))
                {
                    ...
                    // Cr�e un std::vector<UINT8> pour stocker les donn�es de l'image
                    ...
                    if (SUCCEEDED(hr))
                    {
                        // imageData contient maintenant les donn�es de l'image au format std::vector<UINT8>
                        return imageData;
                    }
                }
            }
